...
function endDrag() {
  ...
  dragging = false;
  animating.value = true;
  if (offset.value > height * DRAWER_DISMISS_RATIO) open.value = false;
  else offset.value = 0;
}
...
<template>
  <Teleport to="body">
    <div v-if="rendered" class="drawer-root">
      <div class="backdrop" :style="backdropStyle" @click="open = false" />
